...
export default class Map extends React.Component {
  constructor(props) {
    super(props)
    this.onTouch = debounce(props.onTouch, 16, {
      leading: true,
      trailing: false,
    })
    this.initialRegion = {
      latitude: 46.165484728584715,
      latitudeDelta: 0.2637706968428404,
      longitude: 6.1410642229020596,
      longitudeDelta: 0.22025328129529953,
    }
    this.state = {
      userLocation: null,
    }
  ...
  showAllMarkers(delay) {
    const fn = _ => {
      ...
      const bottom =
        width > height
          ? height * 0.1
          : height - height / 1.61803 + height * 0.0125
      const edgePadding = {
        top: height * 0.1,
        right: width * 0.1,
        bottom,
        ...
        <MapView
          ...
          onMapReady={_ => {
            request(
              ...
              {
                title: 'Belvédère Sonore Geneva',
                message:
                  'Please make sure that your phone always allows location tracking, or else the app can not function properly.',
              },
            )
            this.showAllMarkers(1000)
          }}
